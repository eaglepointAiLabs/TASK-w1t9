...
- Reconciliation CSV imports and manager bulk menu update mutations can be queued as async ops jobs; workers execute real domain logic through service handlers.
- `backup_jobs` and `restore_runs` capture encrypted local backup and restore-test history.
- A maintenance scheduler tick runs inside request lifecycle and performs two automated tasks: due nightly backup creation and bounded queued-job processing.
